...
public final class DexMerger {
    ...
    private DexMerger(Dex[] dexes, CollisionPolicy collisionPolicy, DxContext context, WriterSizes writerSizes)
            throws IOException {
        this.dexes = dexes;
    ...
    public Dex merge() throws IOException {
        ...
        /*
         * We use pessimistic sizes when merging dex files. If those sizes result in too
         * many bytes wasted, compact the result. To compact, simply merge the result
         * with itself.
         */
        ...
        if (wastedByteCount > +compactWasteThreshold) {
            DexMerger compacter = new DexMerger(new Dex[] { dexOut, new Dex(0) }, CollisionPolicy.FAIL, context,
                    compactedSizes);
            result = compacter.mergeDexes();
            context.out.printf("Result compacted from %.1fKiB to %.1fKiB to save %.1fKiB%n", dexOut.getLength() / 1024f,
                    result.getLength() / 1024f, wastedByteCount / 1024f);
        }

        long elapsed = System.nanoTime() - start;
        for (int i = 0; i < dexes.length; i++) {
            context.out.printf("Merged dex #%d (%d defs/%.1fKiB)%n", i + 1,
                    dexes[i].getTableOfContents().classDefs.size, dexes[i].getLength() / 1024f);
        }
        context.out.printf("Result is %d defs/%.1fKiB. Took %.1fs%n", result.getTableOfContents().classDefs.size,
    ...
    /**
     * Reads an IDs section of two dex files and writes an IDs section of a merged
     * dex file. Populates maps from old to new indices in the process.
     */
    abstract class IdMerger<T extends Comparable<T>> {
        ...
        /**
         * Merges unsorted sections by reading them completely into memory and sorting
         * in memory.
         */
    ...
    private SortableType[] getSortedTypes() {
        ...
        /*
         * Populate the depths of each sortable type. This makes D iterations through
         * all N types, where 'D' is the depth of the deepest type. For example, the
         * deepest class in libcore is Xalan's KeyIterator, which is 11 types deep.
         */
        while (true) {
    ...
    /**
     * Reads just enough data on each class so that we can sort it and then find it
     * later.
     */
     ...
     *
     * TODO: this may write multiple copies of the same annotation set. We should
     * shrink the output by merging rather than unioning
     */
    ...
    private void transformClassDef(Dex in, ClassDef classDef, IndexMap indexMap) {
        ...
        if (classDataOff == 0) {
            ...
            transformClassData(in, classDef, classData, indexMap);
        }
    ...
    private void transformClassData(Dex in, ClassDef classDef, ClassData classData, IndexMap indexMap) {
        contentsOut.classDatas.size++;

        if (transformClassDataInject(in, classDef, classData, indexMap))
            return;
    ...
    private void transformCode(Dex in, Code code, IndexMap indexMap) {
        ...
        if (tries.length > 0) {
             ...
             * Unfortunately they're in the opposite order in the dex file so we need to
             * transform them out-of-order.
             */
    ...
    private void initializeInjector() {
        ...
        for (Dex dex : dexes)
            injections.addAll(DexUtils.extractInjections(dex));

        Iterator<Injection> it = injections.iterator();
        while (it.hasNext()) {
            Injection injection = it.next();
            for (int i = 0; i < dexes.length; ++ i) {
                if (dexes[i] == injection.shadowDex)
                    injection.shadowDexIdx = i;
            }
        }

        System.out.println("Blade injector initialized");
    }

    /**
     * Move all member ids from shadow class to target class
     */
    private void fixClassReferences() {

        List<String> strings = dexOut.strings();
        List<Integer> typeIds = dexOut.typeIds();

        for (Injection info : injections) {

            int shadowClassIdx = -1;
            int targetClassIdx = -1;

            for (int i = 0; i < typeIds.size(); ++i) {
                String desc = strings.get(typeIds.get(i));
                if (desc.equals(info.shadowClassName))
                    shadowClassIdx = i;
                if (desc.equals(info.targetClassName))
                    targetClassIdx = i;
            }

            if (shadowClassIdx == -1 || targetClassIdx == -1)
                continue;

            /**
             * All reference to shadow class change to target class And the shadow class
             * will be removed from dex. see readSortableTypes
             */
            IndexMap indexMap = indexMaps[info.shadowDexIdx];
            for (int oldIdx = 0; oldIdx < indexMap.typeIds.length; ++oldIdx) {
                if (indexMap.typeIds[oldIdx] == shadowClassIdx)
                    indexMap.typeIds[oldIdx] = (short) targetClassIdx;
            }
        }
    }

    private List<FieldInfo> moveFields(Dex in, IndexMap indexMap, ClassData.Field[] fields, Injection info,
            ClassData.Field[] shaodwFields) {
        List<FieldInfo> fieldInfos = new ArrayList<>(fields.length);

        for (ClassData.Field f : fields)
            fieldInfos.add(new FieldInfo(in, indexMap, f));

        for (ClassData.Field f : shaodwFields) {
            if (!DexUtils.fieldExists(in, fields, info.shadowDex, f)) {
                fieldInfos.add(new FieldInfo(info.shadowDex, indexMaps[info.shadowDexIdx], f));
            }
        }
    ...
    private List<MethodInfo> moveMethods(Dex in, IndexMap indexMap, ClassData.Method[] methods, Injection info,
            ClassData.Method[] shadowMethods) {
        List<MethodInfo> methodInfos = new ArrayList<>();

        for (ClassData.Method m : shadowMethods) {
            methodInfos.add(new MethodInfo(info.shadowDex, indexMaps[info.shadowDexIdx], m));
        }

        return methodInfos;
    }

    private void transformFields(List<FieldInfo> fields) {
        int lastOutFieldIndex = 0;
        for (FieldInfo field : fields) {
            int outFieldIndex = field.indexMap.adjustField(field.field.getFieldIndex());
            classDataOut.writeUleb128(outFieldIndex - lastOutFieldIndex);
            lastOutFieldIndex = outFieldIndex;
            classDataOut.writeUleb128(field.field.getAccessFlags());
        }
    }

    // Fix invoke-virtual => invoke-direct
    private Code fixDirectMethodCall(Code code, List<MethodInfo> directMethods) {
        DecodedInstruction[] decodedInstructions = DecodedInstruction.decodeAll(code.getInstructions());
        boolean fixed = false;
        for (int i = 0; i < decodedInstructions.length; ++i) {
            DecodedInstruction DI = decodedInstructions[i];
            if (DI != null && DI.getIndexType() == IndexType.METHOD_REF && DI.getOpcode() == Opcodes.INVOKE_VIRTUAL) {
                int methodId = DI.getIndex();
                boolean fixIt = false;
                for (MethodInfo mi : directMethods) {
                    if (mi.method.getMethodIndex() == methodId) {
                        fixIt = true;
                    }
                }
                ...
                if (fixIt) {
                    try {
                        java.lang.reflect.Field index = DI.getClass().getSuperclass().getDeclaredField("opcode");
                        index.setAccessible(true);
                        index.setInt(DI, Opcodes.INVOKE_DIRECT);
                        fixed = true;
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        if (fixed) {
            ShortArrayCodeOutput out = new ShortArrayCodeOutput(decodedInstructions.length);
            for (DecodedInstruction instruction : decodedInstructions) {
                if (instruction != null) {
                    instruction.encode(out);
                }
            }
            
            return new Code(code.getRegistersSize(), code.getInsSize(), code.getOutsSize(), code.getDebugInfoOffset(), out.getArray(), code.getTries(), code.getCatchHandlers());
        }

        return code;
    }

    private void transformMethods(List<MethodInfo> methods, List<MethodInfo> directMethods) {
        int lastOutMethodIndex = 0;
        for (MethodInfo method : methods) {
            int outMethodIndex = method.indexMap.adjustMethod(method.method.getMethodIndex());
            classDataOut.writeUleb128(outMethodIndex - lastOutMethodIndex);
            lastOutMethodIndex = outMethodIndex;

            classDataOut.writeUleb128(method.method.getAccessFlags());

            if (method.method.getCodeOffset() == 0) {
                classDataOut.writeUleb128(0);
            } else {
                codeOut.alignToFourBytesWithZeroFill();
                classDataOut.writeUleb128(codeOut.getPosition());
                Code code = fixDirectMethodCall(method.codeDex.readCode(method.method), directMethods);
                transformCode(method.codeDex, code, method.codeIndexMap);
            }
        }
    }

    private void swapMethod(Injection info, List<MethodInfo> directMethods, List<MethodInfo> virtualMethods) {

        for (SwapMethod si : info.swaps) {
            MethodInfo from = null;
            MethodInfo to = null;
            List<MethodInfo> fromMethods = null;
            List<MethodInfo> toMethods = null;

            for (MethodInfo _mi : directMethods) {
                String desc = _mi.getDescriptor();
                if (desc.equals(si.from)) {
                    from = _mi;
                    fromMethods = directMethods;
                }
                if (desc.equals(si.to)) {
                    to = _mi;
                    toMethods = directMethods;
                }
            }

            for (MethodInfo _mi : virtualMethods) {
                String desc = _mi.getDescriptor();
                if (desc.equals(si.from)) {
                    from = _mi;
                    fromMethods = virtualMethods;
                }
                if (desc.equals(si.to)) {
                    to = _mi;
                    toMethods = virtualMethods;
                }
            }

            if (from != null) {
                if (to != null) {
                    int bak = to.method.getCodeOffset();
                    to.method.setCodeOffset(from.method.getCodeOffset());
                    from.method.setCodeOffset(bak);

                    bak = to.method.getAccessFlags();
                    to.method.setAccessFlags(from.method.getAccessFlags());
                    from.method.setAccessFlags(bak);

                    Dex dexBak = to.codeDex;
                    to.codeDex = from.codeDex;
                    from.codeDex = dexBak;

                    IndexMap codeIM = to.codeIndexMap;
                    to.codeIndexMap = from.codeIndexMap;
                    from.codeIndexMap = codeIM;

                    if (fromMethods != toMethods) {
                        fromMethods.remove(from);
                        fromMethods.add((to));
    ...
    private void sortFields(List<FieldInfo> fields) {
        Collections.sort(fields, new Comparator<FieldInfo>() {

            @Override
            public int compare(FieldInfo arg0, FieldInfo arg1) {
                return arg0.indexMap.adjustField(arg0.field.getFieldIndex()) - arg1.indexMap.adjustField(arg1.field.getFieldIndex());
            }
        });
    }

    private void sortMethods(List<MethodInfo> methods) {
        Collections.sort(methods, new Comparator<MethodInfo>() {

            @Override
            public int compare(MethodInfo arg0, MethodInfo arg1) {
                return arg0.indexMap.adjustMethod(arg0.method.getMethodIndex()) - arg1.indexMap.adjustMethod(arg1.method.getMethodIndex());
            }
        });
    }

    private boolean transformClassDataInject(Dex in, ClassDef classDef, ClassData classData, IndexMap indexMap) {
        Injection info = null;;
        for (Injection _info : injections) {
            if (classDef.toString().startsWith(_info.targetClassName))
                info = _info;
        }
        ...
        List<FieldInfo> instanceFields = moveFields(in, indexMap, classData.getInstanceFields(), info, shadowClassData.getInstanceFields());

        List<MethodInfo> directMethods = moveMethods(in, indexMap, classData.getDirectMethods(), info, shadowClassData.getDirectMethods());
        List<MethodInfo> virtualMethods = moveMethods(in, indexMap, classData.getVirtualMethods(), info, shadowClassData.getVirtualMethods());
        ...
        for (ClassData.Method m : classData.getDirectMethods())
            directMethods.add(new MethodInfo(in, indexMap, m));
        ...
        sortMethods(directMethods);
        sortMethods(virtualMethods);

        classDataOut.writeUleb128(staticFields.size());
        classDataOut.writeUleb128(instanceFields.size());
        classDataOut.writeUleb128(directMethods.size());
        classDataOut.writeUleb128(virtualMethods.size());

        transformFields(staticFields);
        transformFields(instanceFields);

        // After swaped, some virtual method may be converted to direct method
        List<MethodInfo> directMethodsToFix = new ArrayList<>(directMethods.size());
        for (MethodInfo mi : directMethods) {
            if (!AccessFlags.isPrivate(mi.initialAccessFlags)) {
                directMethodsToFix.add(mi);
            }
        }

        transformMethods(directMethods, directMethodsToFix);
        transformMethods(virtualMethods, directMethodsToFix);
    ...
    static class SwapMethod {
        ...
        public SwapMethod(String f, String t) {
            from = f;
            to = t;
        }
    }
    ...
    static class Injection {
        ...
        Dex shadowDex;
        int shadowDexIdx;
        List<SwapMethod> swaps = new ArrayList<>();
    }
    ...
    static class FieldInfo {
        ...
        public FieldInfo(Dex d, IndexMap im, ClassData.Field cf) {
            dex = d;
            indexMap = im;
            field = cf;
        }
    ...
    static class MethodInfo {
        ...
        Dex codeDex;
        IndexMap indexMap;
        IndexMap codeIndexMap;
        ClassData.Method method;
        int initialAccessFlags;

        public MethodInfo(Dex d, IndexMap im, ClassData.Method cm) {
            dex = d;
            codeDex = d;
            indexMap = im;
            codeIndexMap = im;
            method = cm;
            initialAccessFlags = cm.getAccessFlags();
        }
